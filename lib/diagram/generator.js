...
function getNomnomlSourceFromNodes(nodes) {
  ...
  return nomnomlSource + project.name + ' ' + project.version + '\n' + today;
}
...
function extractNodeProperties(properties, allBaseProperties, loopbackRelations) {
  ...
  Object.keys(properties).forEach((propertyName) => {
    ...
    if (properties[propertyName].generated) {
    ...
    if (specialProperties) {
      if (properties[propertyName].generated) {
        nodeProperty.name = '¶ ' + propertyName
...
function mapModelsToNodes(loopbackApplication, options) {
  ...
  models.forEach((model) => {
    ...
    //node.specialize = baseNode; //this line make the result worse. See if important ...
    ...
    if (!_.isEmpty(model.relations)) {
      Object.keys(model.relations).forEach((relationName) => {
        ...
        if (!containsObject(relationsToIgnore, rel)) {
          ...
          } else if (rel.type === 'referencesMany' && !rel.polymorphic && !rel.modelThrough) {
            handleHasMany(nodes, node, rel);
...
function generateTable(loopbackApplication) {
  const models = loopbackApplication.models();
  let table = '';

  models.forEach((model) => {
    table += model.definition.name + '\n';
    table += 'nom;type;description\n';

    let properties = model.definition.properties;

    Object.keys(properties).forEach((propertyName) => {
      table += propertyName + ';' + properties[propertyName].type.name + ';' + (properties[propertyName].description ? properties[propertyName].description : '') + '\n';
    });
...
function generate(loopbackApplication, options) {
  if (options.table) {
    return generateTable(loopbackApplication);
  }
